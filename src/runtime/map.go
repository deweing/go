...
func mapaccess1(t *maptype, h *hmap, key unsafe.Pointer) unsafe.Pointer {
	...
	// 比如B=5时，那么m=31，二进制全是1
	// 求bucket num时，将hash与m相与，决定bucket位置
	m := bucketMask(h.B)
	//获取桶
	b := (*bmap)(add(h.buckets, (hash&m)*uintptr(t.bucketsize)))
	// 判断是否正在扩容
	if c := h.oldbuckets; c != nil {
		//如果不是等量扩容，新的bucket数量是老的2倍
		if !h.sameSizeGrow() {
...
bucketloop:
	for ; b != nil; b = b.overflow(t) { //循环下一桶，链表
		for i := uintptr(0); i < bucketCnt; i++ {
			//先判断tophash是否匹配，匹配再继续判断key是否一致，否则到下一个位置
			if b.tophash[i] != top {
				//说明该bucket后面的位置都是空的，直接到一个bucket
...
func evacuate(t *maptype, h *hmap, oldbucket uintptr) {
	...
	if !evacuated(b) {
		...
		for ; b != nil; b = b.overflow(t) {
			...
			for i := 0; i < bucketCnt; i, k, e = i+1, add(k, uintptr(t.keysize)), add(e, uintptr(t.elemsize)) {
				...
				if !h.sameSizeGrow() {
					...
					// 有协程正在遍历map
					// 如果出现相同的key，算出来的hash值不同
					// 只有在float变量的NaN()情况下会出现
					if h.flags&iterator != 0 && !t.reflexivekey() && !t.key.equal(k2, k2) {
						...
						useY = top & 1 //由tophash最低位决定
						top = tophash(hash)
					} else {
						// 正常情况下
						// 取决于新hash值的 oldB + 1 位是0还是1
						if hash&newbit != 0 {
							useY = 1
						}
					}
				...
				if dst.i == bucketCnt { // i==8, 说明要溢出了
					dst.b = h.newoverflow(t, dst.b) //新建一个桶
					dst.i = 0
		...
		// 只清除Key/Value，保留top hash部分，表示搬迁状态
